...
import {  Dimensions, Modal, RefreshControl, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native'
...
function HistoryCard(props) {
  ...
  // console.log(userProfile && userProfile.filter);
    ...
    const postReview = () => { 
      ...
      axios
        .put(
          `${baseURL}schedule/review/${historyId}`,
          formData,
          config
        )
        .then((res) => {
                        ...
                        >
                            <Text style={styles.title}>{history.category}</Text>
                                ...
                                <View style={{flexDirection:'row'}}>
                                    ...
                                    <Text style={styles.description}> {moment(history.date_schedule).format("MMM Do YY")} </Text>
                                </View>
                                ...
                                <View style={{flexDirection:'row'}}>
                                    ...
                                    {history.status === "attended" ?  (
                                        <View style={{flexDirection:'row'}}>                                      
                                              <Text style={styles.attended} >{history.status.toUpperCase()}</Text> 
                                              {history.review === undefined ? ( 
                                                <TouchableOpacity 
                                                onPress={() => [setModalVisible(true), setHistoryId(history._id) , setStarRating(null), setComment(null)]}>
                                                    <Icon name="star" style={styles.iconStar} size={20}  />
                                                    <Text style={styles.iconlabel}>Review</Text> 
                                                </TouchableOpacity> 
                                     ) :   null}   
                                        </View>
                                    ) :  <Text style={styles.notattended} >{history.status.toUpperCase()}</Text>} 
                                </View> 
                        </Card>
